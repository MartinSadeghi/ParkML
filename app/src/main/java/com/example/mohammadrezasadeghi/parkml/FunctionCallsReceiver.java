...
public class FunctionCallsReceiver extends BroadcastReceiver {
    ...
    public FunctionCallsReceiver(HomeActivity homeActivity) {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (Constants.DID_ENTER_CAR.equals(action)) {
            ...
            Call<ApiResponse> res = intf.api(postData);
            res.enqueue(new Callback<ApiResponse>() {
                @Override
                public void onResponse(Call<ApiResponse> call, Response<ApiResponse> response) {
                    Log.d("FunctionCallsReceiver", "Response: " + response.body().getResults());
                    if ("YES".equals(response.body().getResults())) {
                        homeActivity.openDialog3();
                    } else {
